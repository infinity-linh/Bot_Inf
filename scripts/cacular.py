...
def cacular_speed(areas):
    if areas < 400:
        ...
        different = 25000 - areas
        print("Diff: ", different)
        if different > 0:
            speed = convert_map(different, 25000, 0, 400, 20)
            # print(speed)
            v = '0'*(3-len(str(int(speed)))) + str(int(speed))
            return '200200'+v
    return '200200150'
...
def state_robot(boxes_one, boxes_two, model_ctr, flag_move, state_stop, catch_com, angles_current, state_move, idx):
    ...
    if len(boxes_one) != 0 and len(boxes_two) != 0:
        ...
        if 20000 < area_one < 45000 or 20000 < area_two < 45000:
    ...
    if (len(boxes_one) != 0):
        ...
        # state_move = False
    if (catch_com == True and detect_object == False):
        state_move = True
...
def donghocnguoc(x, y, z, v):
    ...
    return np.array([int(a_r0*180/np.pi), int(a_r1*180/np.pi), int(a_r2*180/np.pi), int(v*180/np.pi)])
